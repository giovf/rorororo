...
- 2026-09-18: Provisioned with owner's keys — Stripe product prod_VHaBfUrfx5ElJ6, price $12, Payment Link buy.stripe.com/6oUdR2gdfcOtaJU22aefC00 (ToS consent on; automatic tax off — Stripe Tax not enabled; Managed Payments status unknown), webhook registered; Cloudflare worker foundry-licenses live at faceless-api.workers.dev with KV + secrets; Resend key is send-only (fine). LICENSE_SERVER wired. Coupons Write missing on the Stripe key.
- 2026-09-18: Resend sender fixed (mail.gankdat.com); licence email delivered via worker. Managed Payments enabled by owner; automatic tax on link. read-focus.zip built; owner-action #5 (Chrome submission) written. Pending: Promotion Codes Write → OWNERTEST → £0 purchase.
- 2026-09-18: End-to-end proven with a real £0 checkout (OWNERTEST): Stripe → webhook → worker → Resend email. Action #3 complete. Next: owner screenshots + Chrome submission (#5).
- 2026-09-19: ReadFocus submitted to Chrome Web Store (after switching to per-site optional host permissions). Both ventures in review. Owner directives: prefer ventures that need no owner testing; wants a visibility interface + request inbox; open to an agent-company structure (discussion pending).
- 2026-09-19: Owner approved: ops dashboard (published as private artifact https://claude.ai/artifact/5EVansYqPeYLQVEcpTG2LU; generator scripts/ops-dashboard.ts; inbox via artifact db), dev container gains Chromium/Playwright/ffmpeg/xvfb/adb (rebuild pending), scheduled routines and per-task sub-agents to follow. Android via adb to host emulator/phone; iOS not possible from the container.
- 2026-09-19: Container rebuilt with Chromium/Playwright/ffmpeg/xvfb/adb — verified. ReadFocus e2e harness (test build + fixture + real-key checks) passes; it caught and fixed a real bug (bolding continued after switch-off). Cloud routines blocked until the owner connects GitHub to claude.ai (owner chose to connect first). V3 discovery started.
- 2026-09-19: V3 Highlight Keep built end to end (73 unit tests, 10 e2e checks), Stripe product prod_VI4BQSx6ML73fr + Payment Link buy.stripe.com/28E14g8KN7u95pA9uCefC02, landing page, listing, zips (Chrome + Firefox lint-clean). Owner action #7 (upload) READY. Firefox build for ReadFocus lint-clean; action #6 for AMO/Edge accounts.
- 2026-09-19: Owner completed AMO keys + CWS OAuth (refresh token stored; auth code removed from .env). ReadFocus + Highlight Keep submitted to Firefox Add-ons via web-ext (listed). Edge blocked by Microsoft account issues. Support inbox worker deployed; awaiting action #8 permissions.
- 2026-09-19: Gmail connector live → read Figma rejection (crash fixed, v0.1.1, resubmit pending), Chrome fee receipt (ledger: £3.70 cost), Google Payments ID verification request (action #9), AMO submission confirmations for both extensions (URLs in STORE.md). CWS Publish API verified against ReadFocus item.
- 2026-09-19: Owner: Gmail send-as info@ done; Figma v0.1.1 resubmitted; Google ID verification submitted; Edge account creation blocked by Microsoft (parked). Resend root domain verified; support sender live. incoming/ drop zone created for the owner's API-led project.
