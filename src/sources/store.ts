...
import { readCached, refreshSource } from './cache';
import { queryD1Source, refreshD1Source, statsD1Source } from './d1store';
...
export async function sourceStats(
  ...
  if (source.storage === 'd1') {
    return statsD1Source(env, source);
  }
  const payload = await readCached(env, source);
  return {
    stats: computeStats(payload.records, source.stats),
    last_refreshed_at: payload.last_refreshed_at,
  };
}
